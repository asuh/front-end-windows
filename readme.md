...
This document assumes you're running a fresh copy of **Windows 10**.
...
By default, Windows comes with two command line tools: Command Prompt and Powershell. Powershell is a task automation and configuration management framework that includes command-line shell. 

Because many tools recommended can be used with another shell called Bash, which contains the command language used to interact with Unix functions, such as `pwd`, `ls`, and `cd`, it is recommended to either install Bash into Windows (coming soon natively in Windows Redstone update - Summer 2016) or use something like [Cmder](http://cmder.net/). Cmder is a powerful console emulator that provides a shell for Bash.
...
Note: It is unlikely you need to install Adobe Flash or Java anymore. Google Chrome contains both packages by default and running independent versions of each is a security risk.
...
Once installed, you can easily install many [versions of Internet Explorer from the Modern.ie site](https://developer.microsoft.com/en-us/microsoft-edge/tools/vms/windows/).
...
Installing it is very easy through Chocolatey:
